...
class VanilaOption :
    ...
    def menu_pricing_vanilla_options(self):
        ...
        price, _ = heston(self.heston_params, S0, K, T, r, q, option_type, n_simulations=20000)
    ...
    def calculate_greeks_vanilla(self, S0, K, T, r, q, option_type):
        ...
        if greeks_choice == "1":
            ...
            S_range = np.linspace(0.2 * S0, 1.8 * S0, 70)
...
class TradingStrategies:
    ...
    def pricing_butterfly(self):
        ...
        if strategy_type == "call butterfly":
            price_butterfly,_ = (
                heston(self.heston_params, S0, K1, T, r, q, "call", n_simulations=30000)
                - 2 * heston(self.heston_params, S0, K2, T, r, q, "call", n_simulations=30000) 
                + heston(self.heston_params, S0, K3, T, r, q, "call", n_simulations=30000)
            )    
        elif strategy_type == "put butterfly":
            price_butterfly,_ = (
                heston(self.heston_params, S0, K1, T, r, q, "put", n_simulations=30000)
                - 2 * heston(self.heston_params, S0, K2, T, r, q, "put", n_simulations=30000) 
                + heston(self.heston_params, S0, K3, T, r, q, "put", n_simulations=30000)
            )
        else : 
...
def plot_vanilla(heston_params, S, K, T, r, q, option_type, position):
    ...
    plt.plot(S_range, prices, label="Option Price", color="blue")
    plt.plot(S_range, payoffs, label="Option Payoff (Monte-Carlo)", color="green", linestyle="--")
    plt.plot(S_range, bs_prices, label="Option Payoff (Black_Scholes)", color="red", linestyle="--", linewidth=0.75)
